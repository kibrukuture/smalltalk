...
}: // remotePeerVideoCalling,
{
  ...
  // useEffect(() => {
  //   // set background time
  //   backgroundTimeInterval = setInterval(() => {
  //     backgroundTime.current += 1;
  //   }, 1000);
    ...
    if (typeof navigator !== 'undefined') {
      (async () => {
        ...
        socket.on('VideoCallAccepted', (data) => {
          ...
          // setCaller(caller);
          console.log('#######33Video call accepted by#########: ', friend.name);
        });
      })();
    }
    return () => {
  ...
  useEffect(() => {
    socket.on('RemotePeerVideoCallEnd', (data) => {
      ...
      // console.log('the local peer', localPeer);
      console.log('another effect: local peer here is : ', localPeer);
    });
  }, [videoStream]);
  ...
  const onLeaveVideoCall = () => {
    ...
    socket.emit('RemotePeerVideoCallEnd', {
      roomId: currentOpenChatId,
      callEndedBy: user,
      peer: friend,
    });
  ...
  console.log('#######:In Video: Local Peer:#######', localPeer);
  return (
